...
class RealityStreamSettings extends XrayCommonClass {
    constructor(show=false, dest='', xver=0, serverNames=[''], privateKey='', minClientVer='', maxClientVer='', maxTimeDiff=0, shortIds=[''], fingerprint='chrome', publicKey='') {
        super();
        this.show = show;
        this.dest = dest;
        this.xver = xver;
    ...
    toJson() {
        return {
            ...
            dest: this.dest,
            xver: this.xver,
            serverNames: this.serverNames.filter(v => !ObjectUtil.isEmpty(v)),
            privateKey: this.privateKey,
            minClientVer: this.minClientVer,
            maxClientVer: this.maxClientVer,
            maxTimeDiff: this.maxTimeDiff,
            shortIds: this.shortIds.filter(v => !ObjectUtil.isEmpty(v)),
        };
    }
}
...
class Inbound extends XrayCommonClass {
    constructor(port=RandomUtil.randomIntRange(10000, 60000),
        ...
        this.normalizeProtocolSettings();
    }
    ...
    validateTlsConfig() {
        if (!this.isTlsRequired()) return null;
        if (this.reality) return null; // Reality doesn't need cert
        const cert = this.stream.tls.certs[0];
        if (cert.useFile) {
            if (!cert.certFile || !cert.keyFile) {
                return '此协议必须开启 TLS，请配置证书文件路径或使用 ACME 申请证书';
            }
        } else {
            if (!cert.cert || !cert.key) {
                return '此协议必须开启 TLS，请填写证书内容或使用 ACME 申请证书';
            }
        }
        return null;
    }
    ...
    canSetTls() {
        if (this.isTlsRequired()) return false;
    ...
    canEnableReality() {
        ...
        return this.network === "tcp" || this.network === "http" || this.network === "grpc" || this.network === "httpupgrade" || this.network === "splithttp";
    }
    ...
    canUseNetwork(network) {
        // QUIC 已被 Xray-core 弃用，不允许新配置使用
        return network !== 'udp' && network !== 'quic';
    }

    isFixedNetworkProtocol() {
        switch (this.protocol) {
        }
        return false;
    }
    ...
    genVLESSLink(address = '', remark='') {
        ...
        if (this.stream.security === 'tls') {
            ...
            const alpn = this.getShareAlpn();
            if (!ObjectUtil.isEmpty(alpn)) {
                params.set("alpn", alpn);
            }
        } else if (this.stream.security === 'reality') {
            const reality = this.stream.reality;
            const serverName = reality.serverNames.find(v => !ObjectUtil.isEmpty(v));
            if (!ObjectUtil.isEmpty(serverName)) {
                params.set("sni", serverName);
            }
            const shortId = reality.shortIds.find(v => !ObjectUtil.isEmpty(v));
            if (!ObjectUtil.isEmpty(shortId)) {
    ...
    genSocksLink(address='', remark='') {
        let settings = this.settings;
        let user = '', pass = '';
        if (settings.auth === 'password' && settings.accounts.length > 0) {
            user = settings.accounts[0].user;
            pass = settings.accounts[0].pass;
        }
        const link = `socks://${encodeURIComponent(user)}:${encodeURIComponent(pass)}@${address}:${this.port}`;
        const url = new URL(link);
        url.hash = encodeURIComponent(remark);
        return url.toString();
    }
    ...
    genHttpLink(address='', remark='') {
        ...
        const link = `http://${encodeURIComponent(user)}:${encodeURIComponent(pass)}@${address}:${this.port}`;
        const url = new URL(link);
        url.hash = encodeURIComponent(remark);
        return url.toString();
    }

    setCommonShareParams(url) {
        if (this.stream && this.stream.security === 'tls') {
            const server = this.stream.tls.server;
            if (!ObjectUtil.isEmpty(server)) {
                url.searchParams.set('sni', server);
            }
            const alpn = this.getShareAlpn();
            if (!ObjectUtil.isEmpty(alpn)) {
    ...
    getShareAlpn(defaultAlpn='') {
        if (!this.stream || !this.stream.tls) {
            return defaultAlpn;
        }
        const alpn = this.stream.tls.alpn;
        if (ObjectUtil.isEmpty(alpn)) {
            return defaultAlpn;
        }
        return alpn instanceof Array ? alpn.join(',') : alpn;
    }

    setStreamShareParams(url) {
        const network = this.stream.network;
        if (this.stream.security !== 'none') {
            url.searchParams.set('security', this.stream.security);
        }
        url.searchParams.set('type', network === 'http' ? 'h2' : network);
        ...
        switch (network) {
            case 'tcp': {
                ...
                if (tcp.type === 'http') {
                    const request = tcp.request;
                    url.searchParams.set('headerType', 'http');
                    url.searchParams.set('path', request.path.join(','));
                    const index = request.headers.findIndex(header => header.name.toLowerCase() === 'host');
                    if (index >= 0) {
                        url.searchParams.set('host', request.headers[index].value);
                    }
                }
                break;
            }
            case 'ws': {
                url.searchParams.set('path', this.stream.ws.path);
                const index = this.stream.ws.headers.findIndex(header => header.name.toLowerCase() === 'host');
                if (index >= 0) {
                    url.searchParams.set('host', this.stream.ws.headers[index].value);
                }
                break;
            }
            case 'http':
                url.searchParams.set('path', this.stream.http.path);
                url.searchParams.set('host', this.stream.http.host.join(','));
                break;
            case 'quic':
                url.searchParams.set('quicSecurity', this.stream.quic.security);
                url.searchParams.set('key', this.stream.quic.key);
                url.searchParams.set('headerType', this.stream.quic.type);
                break;
            case 'grpc':
                url.searchParams.set('serviceName', this.stream.grpc.serviceName);
                break;
            case 'httpupgrade':
                url.searchParams.set('path', this.stream.httpupgrade.path);
                url.searchParams.set('host', this.stream.httpupgrade.host);
                break;
            case 'splithttp':
                url.searchParams.set('path', this.stream.splithttp.path);
                url.searchParams.set('host', this.stream.splithttp.host);
                break;
        }

        if (this.stream.security === 'tls') {
            this.setCommonShareParams(url);
        } else if (this.stream.security === 'reality') {
            const reality = this.stream.reality;
            const serverName = reality.serverNames.find(v => !ObjectUtil.isEmpty(v));
            if (!ObjectUtil.isEmpty(serverName)) {
            ...
            const shortId = reality.shortIds.find(v => !ObjectUtil.isEmpty(v));
            if (!ObjectUtil.isEmpty(shortId)) {
                url.searchParams.set('sid', shortId);
            }
            if (!ObjectUtil.isEmpty(reality.publicKey)) {
                url.searchParams.set('pbk', reality.publicKey);
            }
...
Inbound.RawSettings = class extends Inbound.Settings {
    constructor(protocol, value={}) {
        super(protocol);
        this.value = value || {};
    }

    static fromJson(protocol, json={}) {
        return new Inbound.RawSettings(protocol, json || {});
    }
...
Inbound.ShadowsocksSettings = class extends Inbound.Settings {
    constructor(protocol,
        ...
        this.method = method;
        if (password === '') {
            password = this.isSS2022() ? RandomUtil.randomBase64(32) : RandomUtil.randomSeq(10);
        }
        this.password = password;
        this.network = network;
    }
