...
func (s *Server) initRouter() (*gin.Engine, error) {
	...
	engine.Use(s.csrfTokenGuard())
	engine.Use(s.csrfOriginGuard())
...
func (s *Server) csrfTokenGuard() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !session.ValidateCSRFToken(c, token) {
...
func (s *Server) csrfOriginGuard() gin.HandlerFunc {
	return func(c *gin.Context) {
		method := c.Request.Method
		if method != http.MethodPost && method != http.MethodPut && method != http.MethodPatch && method != http.MethodDelete {
			c.Next()
		...
		if origin != "" {
			if !isSameHost(origin, c.Request.Host) {
				c.AbortWithStatus(http.StatusForbidden)
				return
			}
			c.Next()
		...
		if referer != "" && !isSameHost(referer, c.Request.Host) {
			c.AbortWithStatus(http.StatusForbidden)
			return
		}
		if referer == "" && session.IsLogin(c) {
			c.AbortWithStatus(http.StatusForbidden)
			return
		}
		c.Next()
...
func isSameHost(rawURL string, host string) bool {
...
func (s *Server) Start() (err error) {
	...
	s.cookieSecure = certFile != "" && keyFile != ""
