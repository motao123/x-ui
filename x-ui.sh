...
reset_user() {
	...
	echo && echo -n -e "输入新用户名: " && read username
	echo && echo -n -e "输入新密码 (至少8位，需包含大写、小写、数字、符号中的3类): " && read -s password && echo ""
	if [[ -z "${username}" || -z "${password}" ]]; then
		LOGE "用户名和密码不能为空"
		before_show_menu
		return
	fi
	result=$(/usr/local/x-ui/x-ui setting -username "${username}" -password "${password}" 2>&1)
	if echo "${result}" | grep -q "failed"; then
		LOGE "重置失败: ${result}"
		LOGE "密码要求: 至少8位，需包含大写字母、小写字母、数字、符号中的3类"
		before_show_menu
		return
	fi
	LOGI "用户名和密码已重置，现在请重启面板"
    confirm_restart
}
